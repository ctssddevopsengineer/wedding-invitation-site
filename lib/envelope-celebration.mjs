...
export function createEnvelopeCelebration(compact = false) {
  ...
  for (const side of ['left', 'right']) {
    ...
    for (const type of ['firecracker', 'sprinkler', 'ribbon']) {
      const count = type === 'ribbon' ? (compact ? 5 : 8) : (compact ? 24 : 42);
      for (let i = 0; i < count; i++) {
        ...
        particles.push({
          ...
          x, y, gravity,
          delay: type === 'firecracker' ? .15 + wave * 1.35 + (side === 'right' ? .35 : 0) : random() * 3.1,
          duration: type === 'ribbon' ? 2.8 : type === 'sprinkler' ? 1.5 + random() * .6 : 1.8,
