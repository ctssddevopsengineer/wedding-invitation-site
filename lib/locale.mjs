...
function configuredValue(event, language, key, fallback) {
  ...
  if (typeof localized === 'string' && localized.trim() && !/^\{\{[A-Z0-9_]+\}\}$/.test(localized.trim())) return localized;

  const translated = translate(language, fallback);
  if (translated !== fallback) return translated;

  // Proper names, venue names and addresses usually are not translation-catalog
  // keys. When no explicit localized value exists, automatically transliterate
  // Latin-script deployment values into Bengali/Nepali while preserving explicit
  // *_BN / *_NE values as the highest-priority source of truth.
  return transliterate(fallback, language);
}
