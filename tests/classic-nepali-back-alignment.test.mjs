...
test('Classic Nepali back names preserve full Devanagari glyph ink', () => {
  assert.match(backSection, /\[lang="ne"\]\[data-invitation-theme="classic"\][\s\S]*?\.heritageCoupleNames\s*\{[\s\S]*?overflow:\s*visible/);
...
test('Classic Nepali back groom and bride names are optically lowered to the ampersand', () => {
...
test('Classic Nepali back alignment fix remains narrowly scoped', () => {
  assert.doesNotMatch(backSection, /\[lang="bn"\]|\[lang="en"\]/);
  assert.doesNotMatch(backSection, /data-invitation-theme="(?:blush|magenta|navy|plum|saffron)"/);
