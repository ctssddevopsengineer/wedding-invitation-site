...
const css = fs.readFileSync(new URL('../app/saffron-responsive-typography.css', import.meta.url), 'utf8');
const layout = fs.readFileSync(new URL('../app/layout.js', import.meta.url), 'utf8');

test('Saffron Gold responsive typography is theme-scoped from 681px upward', () => {
...
test('Saffron Gold responsive typography covers all four pages', () => {
  ...
  ]) assert.ok(css.includes(selector), `missing Saffron Gold selector ${selector}`);
});

test('Saffron Gold back page has a tablet-specific fit without moving artwork', () => {
...
test('Saffron Gold native-script names have dedicated Bengali/Nepali treatment', () => {
  assert.match(css, /:is\(\[lang="bn"\], \[lang="ne"\]\)\[data-invitation-theme="saffron"\][\s\S]*?dynamicFrontNames/);
  assert.match(css, /:is\(\[lang="bn"\], \[lang="ne"\]\)\[data-invitation-theme="saffron"\][\s\S]*?familyCoupleNames/);
  assert.match(css, /:is\(\[lang="bn"\], \[lang="ne"\]\)\[data-invitation-theme="saffron"\][\s\S]*?heritageCoupleNames/);
});

test('Saffron Gold typography layer does not alter artwork geometry', () => {
  ...
  for (const forbidden of ['top:','left:','right:','bottom:','width:','height:','max-width:','max-height:','min-width:','min-height:','position:','transform:','aspect-ratio:','object-fit:','margin:','padding:','overflow:']) {
    assert.equal(declarations.some(line=>line.startsWith(forbidden)),false,`Saffron Gold typography must not use ${forbidden}`);
  }
});

test('Saffron Gold typography loads after prior theme layers and before compact scrolling', () => {
  ...
  const saffron=layout.indexOf("import './saffron-responsive-typography.css';");
  const compact=layout.indexOf("import './compact-details-scroll.css';");
  assert.ok(magenta>=0 && navy>magenta && plum>navy && saffron>plum && compact>saffron);
});
